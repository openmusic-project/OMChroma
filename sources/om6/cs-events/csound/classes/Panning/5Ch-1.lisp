  ...
  (
   (source-code :initform
                 (load-buffer-textfile
                  (get-orc-source (get-orc "5Ch-1" :local))
                  'textfile "append")
                 :allocation :class :type textfile :accessor source-code)
   (numchan :initform (or (get-orc-channels (get-orc "5Ch-1")) 2) :allocation :class  :accessor numchan)
   (globals-list :initform (get-orc-globals (get-orc "5Ch-1")) :allocation :class :type list :accessor globals-list)
...
;		  [0=1, 72/-288=2, 144/-216=3, 216/-144=4, 288/-72=5]
