...
(defmethod durs->tmstruct (ldurs &optional (begtime 0.0))
  :icon '(141)
...
into a list of time segments (<onset-beg i> <onset-end i>)."
  (let* ((durs (if (listp (car ldurs)) (get-second ldurs) ldurs))
         (onsets (durs->onsets durs :beg begtime)))
    (mapcar #'(Lambda (x y) (list x y)) onsets durs)))

;(durs->tmstruct '(1 1 1 5))
...
(defmethod! durs->onsets (ldurs &optional (beg 0.0)) 
  ...
  (let ((endtimes (if (listp (car ldurs)) (mapcar #'car ldurs) (dx->x beg ldurs))))
    endtimes))
;(durs->onsets '(1 1 1 5))
...
Negative times are eliminated (with warning message). *** STILL TO DO!"
  ...
  (let* ((segs (durs->tmstruct ldurs begtime))
...
;(durs->timestruct '(1 1 1 5) :begtime 0.0)
;(durs->timestruct '(1 1 1 1 1) 0.0 :fact 0.1)
;(durs->timestruct '(1 1 1 1 1) 0.0 :fact 0.1 :op '*)
;(durs->timestruct '(1 1 1 1 1) 0.0 :fact 0.01 :op '* :durmin 0.1)

;(durs->timestruct '((0 1) (2 2) (3 3) (4 5) (5 1)))
;(durs->timestruct '((0 1) (2 1)  (3 1) (4 1) (5 1)) 0.0 :fact '(-0.1 0 0.1))
;(durs->timestruct '(1 1 1 1 1) 0.0 :fact '(0.1 0.2 0.3) :op '*)
;(durs->timestruct '(1 1 1 1 1) 0.0 :fact 0.01 :op '* :durmin 0.1)


(defmethod! segs->durs (lonsets &key (fact ()) (op '+) (durmin (cr::get-gbl 'cr::durmin)) (durmode t))
            (print "Deprecated, use timestruct->durstruct")
            (durstruct->
(defmethod! segs->durs (lonsets &key (fact ()) (op '+) (durmin (cr::get-gbl 'cr::durmin)) (durmode t))
            :icon '(141)
  :indoc '("a list of onsets or time segments [(<beg-onset i> <end-onset i>)]"
