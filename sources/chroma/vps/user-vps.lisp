...
;     '((261 415 587 784 1108) (0.0 -3.2 -5.0 -7.0 -10)) ; fqs + amps
;     '((261 415 587 784 1108) (0.0 -3.2 -5.0 -7.0 -10) (3.0 5.0 7.0 9.0 11.0)) ; fqs + amps +bws
...
(defmethod make_vps ((list list) &optional reference)
  ...
  (if reference
    (case (type-of_vps list reference)
      ...
      (otherwise (error "UNRECOGNIZABLE INPUT STRUCTURE : ~a " list)) )
...
;added ms 091015, allow for FQLs with freqs, amps and bws
  (cond
   ((numberp (car list))
    (make-instance 'fql :the-list list))
;added ms110902, allow also a list with ONE list of freqs only
   ((= (length list) 1)
    (make-instance 'fql :the-list (car list)))
   ((= (Length list) 3) ; there are 3 lists, order must be fqls, amps, bwl
    (make-instance 'fql :the-list (car list) :amplitudes (cadr list) :bwl (caddr list)))
   ((= (length list) 2) ; there are 2 lists, obligatorily fqls and amps
    (make-instance 'fql :the-list (car list) :amplitudes (cadr list)))
   (t
    (error "CANNOT UNDERSTAND THE FORM OF THIS FQL, SIR... : ~a " list))))
...
;(make_fql '((440 550) (-10 -20) (3 4)))
;(make_fql '((440 550) (-10 -20)))
;(make_vps '((DO4 . 50) (RE5 . -33)))
;(interval-p 4)
;(make_vps '(4 4+ 6-)) 
;(make_vps '(4 (3- 0 30))) 
;(make_vps '(4 (3- 0 30))) 
;(make_vps '((3+ 0 50) (3- 0 30)))
;*****************************************************************************
