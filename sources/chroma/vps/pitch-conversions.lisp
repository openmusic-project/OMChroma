...
Further debugged and modified (defun->om::defmethod!) by Marco, May 1998
...
(om::defmethod! fq-to-pch ((f t) &optional (approx 0))
  :icon 141
  (if (pitch-with-octave-p f)
    f
...
;PITCH
(om::defmethod! pch->fq ((note number) &rest diap)        ;nouvelle version pour pch (cf : LLdg-pitch.lisp) 
  :icon 141
  (declare (ignore diap))
  note)

(om::defmethod! pch->fq ((note list) &rest diap)        ;nouvelle version pour pch (cf : LLdg-pitch.lisp) 
...
(om::defmethod! from-midic ((midi number))
  :icon 141
  (if (< midi 127) midi
      (/ midi 100.0)))
;    (round (/ midi 100.0)))) ; WRONG VERSION, corrected by Marco, Feb 21, 2000

(om::defmethod! from-midic ((midi list))
...
(om::defmethod! semitones-to-ratio (ratio)
  :icon 141
  (expt 2 (/ ratio 12)))
...
(om::defmethod! semitones-to-itvl (i)
