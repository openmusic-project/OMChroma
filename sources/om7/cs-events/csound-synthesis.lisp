  ...
  (unwind-protect 
      ...
      (let* ((path-aiff (if (equal :rt name) name
        ...
        (if run   
          ...
          (list (probe-file path-orc) (probe-file path-sco)))
        )
    ...
    (unwind-protect 
        ...
        (let* ((path-aiff (if (equal :rt name) name
                            (if (pathnamep name) name 
                              (om::outfile name :type format))))
               (cs-basename (if (equal :rt name) "cs_temp" (pathname-name path-aiff)))
               (path-orc (om::handle-new-file-exists (om::tmpfile cs-basename :type "orc")))
               (path-sco (om::handle-new-file-exists (om::tmpfile cs-basename :type "sco")))
          ...
          ;;; (re)format cs-inits (mostly used by OMPrisma)
          (setf inits (format-inits (om::flat (om::list! inits)) csound-events))
          
          (unless nch 
            (setf nch (or (get-num-channels-from-inits inits)
                          (get-num-channels-from-inst-list instlist))))
        
          (if (null nch)
              (progn
                (om::om-message-dialog "Error: All instruments must have the same number of channels !")
                nil)
            ...
            (progn ;; => GO
              ...
              (with-open-file (out path-orc :direction :output)
...
(defmethod cs-synthesize ((self cs-evt) &key (name "my-synt") tables (run t) (format "aiff"))
  (cs-synthesize (list self) :name name :tables tables :run run :format format))
